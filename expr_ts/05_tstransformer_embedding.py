...
from vectorrepr.models import TimeSeriesTransformerEmbedding, TimeSeriesCNN, LSTMEncoder
...
state_dict = torch.load("checkpoints/model_epoch4.pth", map_location=device)
# 移除 `module.` 前缀
...
with torch.no_grad():
    while start_idx < len(samples):
        ...
        elif isinstance(model, TimeSeriesCNN):
